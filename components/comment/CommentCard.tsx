  ...
  const { selectedAccount } = useAccount();
	...
	const handleDelete = async (id:number)=>{
		if(window.confirm('本当に削除しますか？')){
         await deleteComment(comment.patientId, Number(id));
		}
	}
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 space-y-2"   >
      ...
      {isEditing ? (
        ...
        <p className="text-gray-700" onClick={() => setIsEditing(selectedAccount?.id === comment.accountId)}
        style={{
          cursor: selectedAccount?.id === comment.accountId? "pointer" : "default"
        } }>{comment.content}</p>
      )}
